...
Made for gforth.
Adopted to win32forth 15.06.2012
...
\ win32forh adaptation of gforth words
: latest  ( -- nfa ) last @ ;
: name>string ( nfa -- n adr ) nfa-count ;
...
\            IF  _comma ELSE type true abort" ?" THEN
          count (number?)  ( adr n -- d f )
            IF d>s _comma ELSE true abort" ???" THEN
        THEN
     REPEAT
     drop
...
\                0 do dup i + c@  toupper emit loop drop      _; \ok
                0 do dup i + c@  upc emit loop drop      _; \ok
