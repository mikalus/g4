...
Note: 
Use of [ ] in g4 has to be inside of one single line so far. 
If you use characters like ( ) ? ! @ ; , : in forth names you have to change them manualy in phrases like 'leftbracket' and so on in the assembler file which is generated by G4. There is no automatic expansion of those special characters in names. 
